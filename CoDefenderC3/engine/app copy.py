...
def _scan_single_pe(
    ...
    if ens_ok:
        ens_score_map = {r["model"]: r["score"] for r in ens_ok}
        ens_pred_map = {r["model"]: r["prediction"] for r in ens_ok}
        ens_scores = np.array(
            [ens_score_map.get(m, 0.5) for m in ens._model_names],
            dtype=np.float32,
        ).reshape(1, len(ens._model_names))
        ens_preds = np.array(
            [ens_pred_map.get(m, 0) for m in ens._model_names],
            dtype=np.int64,
        ).reshape(1, len(ens._model_names))
        ens_pred_arr, ens_score_arr = ens.ensemble_predict(ens_scores)
    else:
...
async def _batch_scan_samples(
    ...
    try:
        ...
        batch_predictions, batch_scores = ens.ensemble_predict(per_model_scores)
...
async def predict_directory(
    ...
    try:
        async with _SCAN_SEMAPHORE:
            ...
            results = await _batch_scan_samples(pe_files, ens, expected_view_dims, include_de)
