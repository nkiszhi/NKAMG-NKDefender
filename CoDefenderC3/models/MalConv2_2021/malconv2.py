...
from models.MalConv2_2021.LowMemConv import LowMemConvBase
from models.MalConv2_2021.MalConvML import MalConvML
from models.MalConv2_2021.Checkpoint import CheckpointFunction
...
class MalConvGCT(LowMemConvBase):
    ...
    def forward(self, x):
        ...
        # 为了计算DFE，缓存中间特征
        self._cached_post_conv = post_conv

        return output, penult, post_conv

    def extract_middle_feature(self):
        """
        提取模型中间特征（post_conv），用于DFE计算。
        必须在 forward() 之后调用，依赖 forward 缓存的结果。
        """
        if hasattr(self, "_cached_post_conv"):
            return self._cached_post_conv.detach().cpu()
        else:
            raise RuntimeError("请先执行 forward()，以缓存 post_conv 特征。")
