...
def drop_zeros_hook(module, grad_input, grad_out):
    ...
    with torch.no_grad():
        for g in grad_input:
            if g is None:
                grads.append(None)
            elif torch.nonzero(g).shape[0] == 0:  # ITS ALL EMPTY!
...
class LowMemConvBase(nn.Module):
    ...
    def __init__(self, chunk_size=65536, overlap=512, min_chunk_size=1024):
        ...
        if hasattr(self.cat, 'register_full_backward_hook'):
            self.cat.register_full_backward_hook(drop_zeros_hook)
        else:
            self.cat.register_backward_hook(drop_zeros_hook)  # PyTorch < 1.8 fallback
        # 感受野、步长和输出通道的占位符
    ...
    def seq2fix(self, x, pr_args=None):
        ...
        x_selected = x_selected.reshape(x_selected.size(0), -1)
